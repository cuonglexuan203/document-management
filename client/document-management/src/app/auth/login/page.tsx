...
import { LoginRequestBody, useSignInMutation } from "@/app/(main)/_store/services/authApi";
import { ROLE, signIn } from "@/app/(main)/_store/features/authSlide";
import { updateUser } from "@/app/(main)/_store/features/userSlide";
...
export default function SignIn() {
    ...
    const handleFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        const inputedLogInData: LoginRequestBody = {
            username,
            password,
        };
        //
        try {
            ...
            if (signInResponse) {
                ...
                dispatch(updateUser(signInResponse))
                dispatch(
                    signIn({
                        id: signInResponse.id,
                        isLogging: true,
                        isAccount: signInResponse.roles.includes(ROLE.User),
                        isSecretary: signInResponse.roles.includes(ROLE.Secrectary),
                        isAdmin: signInResponse.roles.includes(ROLE.Admin)
                    })
                );
            }
        } catch (err) {
    ...
    //
    return (
        <section className="w-full h-screen bg-white flex justify-center items-center flex-1">
